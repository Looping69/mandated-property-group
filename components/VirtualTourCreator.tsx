...
import { Camera, RefreshCw, Save, Trash2, Mic, Play, Aperture, Check, Video, Home, Info, ChevronRight, ChevronLeft, Sparkles, Upload, Film, Settings2 } from 'lucide-react';
import { analyzeTourImage } from '../services/geminiService';
import { TourStop, VirtualTour } from '../types';
...
export const VirtualTourCreator: React.FC = () => {
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      if (!selectedVoiceURI && available.length > 0) {
        ...
        const defaultVoice = available.find(v => v.name.includes('Google US English') || v.name.includes('Samantha')) || available[0];
  ...
  const handlePublishTour = () => {
    ...
    const newTour: VirtualTour = {
      ...
      status: 'published',
      voiceURI: selectedVoiceURI // Save selected voice
    };

    addTour(newTour);
    alert("Virtual Tour Published! You can now view it linked to the property listing.");
    setTourStops([]);
    setTourTitle("");
        ...
        <div className="lg:col-span-4 flex flex-col h-full">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col h-[700px] sticky top-24">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl space-y-4">
                ...
                <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block tracking-widest flex items-center gap-1">
                      <Settings2 size={10} /> Tour Voice Personality
                    </label>
                    <select 
                      className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-brand-green shadow-sm"
            ...
            <div className="p-6 border-t border-slate-100 bg-white rounded-b-2xl">
                ...
                <p className="text-center text-[10px] text-slate-400 mt-4 uppercase font-bold tracking-widest">
                  Secure Cloud Publishing Enabled
                </p>
            </div>
          </div>
        </div>
